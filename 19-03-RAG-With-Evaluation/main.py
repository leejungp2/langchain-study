...
from rag.pdf import PDFRetrievalChain #Retrieval Chain 상속 받아 pdf 문서 로드 및 검색
from langchain_teddynote import logging
from rag.evaluation import RagEvaluator # 응답의 신뢰도, 관련성을 평가
...
def embed_file(file):
    ...
    file_path = f"./.cache/files/{file.name}" ## Streamlit 캐싱을 사용하여 같은 파일을 다시 업로드하면 재처리하지 않음
    with open(file_path, "wb") as f:
        f.write(file_content)
    return file_path


## PDF 문서를 검색할 수 있도록 RAG 체인 생성
def create_rag_chain(file_path):
    # PDF 문서를 로드
    pdf = PDFRetrievalChain([file_path]).create_chain() ## 문서 검색 및 답변 생성 체인 생성

    # retriever 와 chain을 생성
    pdf_retriever = pdf.retriever ## 위 pdf 객체의, retriever 속성을 가져오는 것
    pdf_chain = pdf.chain
